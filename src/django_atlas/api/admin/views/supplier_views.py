...
"""Supplier facade — procurement-kind projection of Source.

A human sees "Supplier", never "procurement source". Thin wrapper around
`source_service` — the underlying model/service is untouched; `kind` is forced to
procurement server-side and never accepted from the client. Nested resources (feeds,
mapping-profiles, credentials, delete-impact, ...) stay under `/sources/{idx}/...` —
this facade is a projection of the Source collection only (YAGNI, see plan-notes §2.10).
...
from django_atlas.schemas.requests.supplier import SupplierCreateRequest, SupplierUpdateRequest
from django_atlas.schemas.responses.supplier import SupplierListResponse, SupplierResponse
from django_atlas.services import source_service

_TAGS = ["Suppliers"]
_KIND = SourceKind.PROCUREMENT.value


def _serialize(s) -> dict:
    return SupplierResponse.model_validate(s).model_dump(mode="json")
...
class SupplierViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        description="Sources with kind=procurement, projected as Supplier.",
        parameters=[
            OpenApiParameter(
        ...
        responses={200: SupplierListResponse},
    )
    ...
    @extend_schema(tags=_TAGS, summary="Retrieve supplier", responses={200: SupplierResponse, 404: None})
    def retrieve(self, request: Request, idx: str) -> Response:
        source = self._get_procurement_source(idx)
    ...
    @extend_schema(
        ...
        responses={201: SupplierResponse, 400: None},
    )
    def create(self, request: Request) -> Response:
        try:
            data = SupplierCreateRequest(**request.data)
    ...
    @extend_schema(
        ...
        responses={200: SupplierResponse, 400: None, 404: None},
    )
    def partial_update(self, request: Request, idx: str) -> Response:
        self._get_procurement_source(idx)
        try:
            data = SupplierUpdateRequest(**request.data)
        except ValidationError as exc:
            raise_pydantic_as_drf(exc)
    ...
    @extend_schema(tags=_TAGS, summary="Soft-delete supplier", responses={200: None, 404: None})
    def destroy(self, request: Request, idx: str) -> Response:
        self._get_procurement_source(idx)
        result = source_service.delete_source(idx, force=False)
        return Response(result)

    def _get_procurement_source(self, idx: str):
        try:
            source = source_service.get_source(idx)
        except ValueError as exc:
            raise drf_exceptions.NotFound(str(exc) or f"Supplier '{idx}' not found") from exc
        if source.kind != _KIND:
            raise drf_exceptions.NotFound(f"Supplier '{idx}' not found")
        return source
